...
import webhooks, json, datetime, queue, webbrowser
...
class TokenManager(object):
  ...
  def fetch(self,
    ...
    print("Hook Run")
    webbrowser.open('https://id.twitch.tv/oauth2/authorize?client_id=7583ak4tqsqbnpbdoypfpg2h0ie4tu&redirect_uri=http://localhost:5000/webhook&response_type=code&scope=openid+channel:read:editors+moderator:manage:automod+moderator:manage:banned_users+channel:read:hype_train+channel:read:polls+channel:manage:polls+channel:read:predictions+channel:read:redemptions+channel:read:subscriptions+moderation:read+user:edit+user:read:broadcast+user:read:email+user:read:follows+user:read:subscriptions+channel:moderate+chat:edit+chat:read+whispers:read+whispers:edit+channel:manage:polls', new=0, autoraise=True)
    token = tokenHook.run()
  ...
  def auto(self):
    ...
    if self.token:
      try:
        print('1')
        self.outQueue.put(self.verifyToken(self.token))
        print("Valid token passed.")
      except self.Exceptions.InvalidToken:
        try:
          print('2')
          self.outQueue.put(self.refresh())
          print("Refreshed token.")
        except self.Exceptions.InvalidRefresh:
          try:
            print('3')
            self.outQueue.put(self.fetch())
            print("Fetched token.")
          except:
            print('4')
